...
>>> fio.assert_path_is_readable(path_flat)  # doctest: +SKIP
Traceback (most recent call last):
...
AssertionError: Cannot read non-existent path: example.txt

```python
>>> path_compressed: Path = tmp_dir / "example.txt.gz"
>>> fio.assert_path_is_readable(path_compressed)  # doctest: +SKIP
Traceback (most recent call last):
...
...
def to_reader(path: Path) -> Union[io.TextIOWrapper, TextIO, IO[Any]]:
    ...
    Example:
        >>> import fgpyo
        >>> reader = fgpyo.io.to_reader(path = Path("reader.txt"))  # doctest: +SKIP
...
def to_writer(path: Path, append: bool = False) -> Union[IO[Any], io.TextIOWrapper]:
    ...
    Example:
        >>> import fgpyo
        >>> writer = fgpyo.io.to_writer(path = Path("writer.txt"))  # doctest: +SKIP
...
def read_lines(path: Path, strip: bool = False) -> Iterator[str]:
    ...
    Example:
        >>> import fgpyo
        >>> read_back = fgpyo.io.read_lines(path)  # doctest: +SKIP
    """
